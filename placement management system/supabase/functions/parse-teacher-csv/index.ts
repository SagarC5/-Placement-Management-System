...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (lines.length < 2) {
      return new Response(
        JSON.stringify({ error: "CSV must contain headers and at least one teacher record" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (teachers.length === 0) {
      return new Response(
        JSON.stringify({ 
          error: "No valid teacher records found",
          details: errors 
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    ...
    // Insert teachers with uploaded_by field
    const teachersWithUploader = teachers.map(t => ({
      ...t,
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to upload teachers", details: error.message }),
    ...
    console.error("Error in parse-teacher-csv function:", error);
    return new Response(
